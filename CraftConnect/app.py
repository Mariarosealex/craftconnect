...
def admin_users():
    ...
    # Ensure the 'approved' column exists in the 'products' table
    try:
    ...
    if user_type == 'manufacturers':
        users = conn.execute("SELECT * FROM users WHERE user_type = 'manufacturer'").fetchall()
    elif user_type == 'customers':
        users = conn.execute("SELECT * FROM users WHERE user_type = 'customer'").fetchall()
    else:
        users = conn.execute("SELECT * FROM users").fetchall()

    conn.close()

    return render_template('admin_users.html', users=users, selected_role=user_type)
